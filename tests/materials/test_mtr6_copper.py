#!/usr/bin/env python3
"""
MTR-6 Copper Thermal Conductivity Calibration - Tests.
...
from backend.progress.mtr6_copper_conductivity import JOB_KIND, run_calibration
...
def _run_mtr6_job(artifact_dir: Path, canary_mode: bool = False):
    job_store = JobStore()
    ...
    job = runner.create_job(payload=MTR6_PAYLOAD)
...
class TestMTR6CopperConductivity:
    """MTR-6 copper conductivity: E2E, V&V, reproducibility."""
    ...
    def test_a_e2e_create_run_normal_canary_artifacts_evidence(self, tmp_path, monkeypatch):
        ...
        trace1, job1 = _run_mtr6_job(tmp_path, canary_mode=False)
        trace2, job2 = _run_mtr6_job(tmp_path, canary_mode=True)
        ...
        for job in (job1, job2):
            ...
            assert job.result.get("mtr_phase") == "MTR-6"
        ...
        res = snap.get("result", {})
        assert res.get("mtr_phase") == "MTR-6"
    ...
    def test_c_reproducibility_same_payload_twice_shape_matches(self, tmp_path, monkeypatch):
        ...
        _run_mtr6_job(tmp_path, canary_mode=False)
        _run_mtr6_job(tmp_path, canary_mode=False)

#!/usr/bin/env python3
"""
MTR-5 Stainless Steel SS316L Young's Modulus Calibration - Tests.
...
from backend.progress.mtr5_steel_calibration import JOB_KIND, run_calibration
...
def _run_mtr5_job(artifact_dir: Path, canary_mode: bool = False):
    job_store = JobStore()
    ...
    job = runner.create_job(payload=MTR5_PAYLOAD)
...
class TestMTR5SteelModulus:
    """MTR-5 steel calibration: E2E, V&V, reproducibility."""
    ...
    def test_a_e2e_create_run_normal_canary_artifacts_evidence(self, tmp_path, monkeypatch):
        ...
        trace1, job1 = _run_mtr5_job(tmp_path, canary_mode=False)
        trace2, job2 = _run_mtr5_job(tmp_path, canary_mode=True)
        ...
        for job in (job1, job2):
            ...
            assert job.result.get("mtr_phase") == "MTR-5"
        ...
        res = snap.get("result", {})
        assert res.get("mtr_phase") == "MTR-5"
    ...
    def test_c_reproducibility_same_payload_twice_shape_matches(self, tmp_path, monkeypatch):
        ...
        _run_mtr5_job(tmp_path, canary_mode=False)
        _run_mtr5_job(tmp_path, canary_mode=False)

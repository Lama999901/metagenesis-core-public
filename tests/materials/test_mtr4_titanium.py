#!/usr/bin/env python3
"""
MTR-4 Titanium Ti-6Al-4V Young's Modulus Calibration - Tests.
...
from backend.progress.mtr4_titanium_calibration import JOB_KIND, run_calibration
...
def _run_mtr4_job(artifact_dir: Path, canary_mode: bool = False):
    job_store = JobStore()
    ...
    job = runner.create_job(payload=MTR4_PAYLOAD)
...
class TestMTR4TitaniumModulus:
    """MTR-4 titanium calibration: E2E, V&V, reproducibility."""
    ...
    def test_a_e2e_create_run_normal_canary_artifacts_evidence(self, tmp_path, monkeypatch):
        ...
        trace1, job1 = _run_mtr4_job(tmp_path, canary_mode=False)
        trace2, job2 = _run_mtr4_job(tmp_path, canary_mode=True)
        ...
        for job in (job1, job2):
            ...
            assert job.result.get("mtr_phase") == "MTR-4"
        ...
        res = snap.get("result", {})
        assert res.get("mtr_phase") == "MTR-4"
        assert "result" in res
        assert "estimated_E" in res["result"]
        assert "rmse" in res["result"]
        assert "relative_error" in res["result"]

    def test_b_vv_relative_error_threshold(self):
        E_true = 114.0e9
        out = run_calibration(
            seed=42,
            E_true=E_true,
    ...
    def test_c_reproducibility_same_payload_twice_shape_matches(self, tmp_path, monkeypatch):
        ...
        _run_mtr4_job(tmp_path, canary_mode=False)
        _run_mtr4_job(tmp_path, canary_mode=False)
        ...
        assert "relative_error" in res0["result"]

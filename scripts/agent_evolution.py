...
def check_claude_md(actual_count):
    ...
    if "v0.7.0" not in content:
        issues.append("CLAUDE.md doesn't mention v0.7.0")
...
def main():
    ...
    print(f"  {datetime.now().strftime('%Y-%m-%d %H:%M')} | v0.7.0")
    ...
    if passed == total:
        ...
        print(f"  /gsd:quick \"Update CLAUDE.md to reflect current state: {count} tests, v0.7.0\"")
        if stale:
